...
public class GUI extends IData {
	...
	public GUI() {
		...
		UIButtons.add(new UIButton(Events.NEXT_TURN, "Next Turn", HEX_RADIUS * 4, HEX_RADIUS, WIDTH - (HEX_RADIUS * 4),
				HEIGHT - HEX_RADIUS));
	}

	/*
	 * public void drawTest(Graphics2D g) { Polygon poly1 = new Polygon();
	 * HexCoordinate h = layout.pixelToHex(new Point(100, 100)); Point[] p =
	 * layout.polygonCorners(h); for (int k = 0; k < p.length; k++) {
	 * poly1.addPoint((int) p[k].x, (int) p[k].y); }
	 * 
	 * Polygon poly2 = new Polygon(poly1.xpoints, poly1.ypoints, poly1.npoints);
	 * poly2.translate(50, 50); g.setColor(Color.RED); g.drawPolygon(poly1);
	 * g.setColor(Color.BLUE); g.drawPolygon(poly2); }
	 */
	...
	public void drawSelectedHex(Graphics2D g) {
		 ...
		 * g.setStroke(new BasicStroke(3.5f));
		 * 
		 * HexCoordinate s = layout.pixelToHex(new Point(mouseX, mouseY)); if
		 * (hexMap.getHex(s) != null) { //Point[] p = layout.polygonCorners(s); Point
		 * pnt = layout.hexToPixel(s); Polygon translatedPoly = new
		 * Polygon(poly.xpoints, poly.ypoints, poly.npoints);
		 * translatedPoly.translate((int) pnt.x, (int) pnt.y);
		 * 
		 * //System.out.println("Drawing @ (" + poly.xpoints[0] + ", " + poly.ypoints[0]
		 * + ") -- offset poly @ (" + translatedPoly.xpoints[0] + ", " +
		 * translatedPoly.ypoints[0] + ")");
		 * 
	...
	public void drawUI(Graphics2D g) {
		...
		if (civs.get(0).getNumberOfCities() > 0) {
			...
			g.drawString(Integer.toString(civGoldTotal) + "(+" + Integer.toString(civGoldPT) + ")", textX += offsetX,
					yieldHeight); // Gold
	...
	/*
	 * public void addFarm() { if (farmToAdd) { farmToAdd = false; int mX =
	 * MouseHandler.movedMX; int mY = MouseHandler.movedMY; FractionalHex fh =
	 * Layout.pixelToHex(layout, new Point(mX - scrollX, mY - scrollY)); Hex h =
	 * FractionalHex.hexRound(fh); int hexKey = HexMap.hash(h); Improvement i = new
	 * Farm(); Hex mapHex = map.get(hexKey); mapHex.setImprovement(i);
